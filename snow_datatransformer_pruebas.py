...
class BinaryToDistanceTransformer(TransformerMixin):
    def __init__(self, p, _norm='l2', _metric='euclidean'):
        self.norm = _norm
        self.metric = _metric
        self.p=p
    ...
    def transform(self, X, y=None, **fit_params):
        ...
        if(self.p==1):
            ...
            X_scaled = preprocessing.scale(X[0]) # hace copia de los datos, posible optimizacion con copy=False, requiere sparce matrix

            X_normalized = preprocessing.normalize(X_scaled, norm=self.norm)

            X_dense = np.matrix(X_normalized).astype('float')
            return [pairwise_distances(X_dense, metric=self.metric),X[1]]
        else:
...
class FiltroNGramasTransformer(TransformerMixin):
    def __init__(self, numMagico, vectorizer,clas_o):
    ...
    def transform(self, X, y=None, **fit_params):
        ...
        for i in range(0, X.shape[0]):
            ...
            if X[i].sum() >= self.numMagico:
                ...
                if(len(self.clas_o)>0):
                    self.clas_original_filtrada.append(self.clas_o[i])
        ...
        return [self.Xclean,self.clas_original_filtrada]
